...
async function startContainer(
  ...
  if (state === "stopped" || state === "running") {
    ...
    if (created.code !== 0)
      throw new Error(`docker run failed: ${created.out.slice(-300)}`);
  }

  const port = await publishedPort(container);
  if (port === null)
    throw new Error(`could not read the published port of ${container}`);
  ...
  while (Date.now() < deadline) {
    try {
      const res = await fetch(
        `${endpoint.url}/api/status?token=${endpoint.token}`,
        {
