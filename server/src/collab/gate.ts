...
class Frame {
   ...
   */
  private pathOfType(type: Y.AbstractType<unknown>): { segs: string[]; cell: Y.Item | null } {
    const segs: string[] = []
  ...
  private place(target: Target, path: string): Place {
    if (target.kind === 'gc') return { container: ['<gc>'], sub: null, cell: null, cellIsFresh: false }
    ...
    if (typeof parent === 'string') {
      here = { container: [parent], sub: struct.parentSub, cell: null, cellIsFresh: false }
      ...
      here = { container: beside.container, sub: beside.sub, cell: beside.cell, cellIsFresh: beside.cellIsFresh }
    }
  ...
  private verdictFor(
    ...
    if (slot[0] === CELLS_KEY) {
      ...
      if (slot.length === 2 && slot[1] === '[]') {
        return { rule: 'structure', verb: op === 'insert' ? 'add' : 'remove', cellId, path }
      }
   ...
   */
  private checkFresh(fresh: Map<Y.Item, Map<string, Y.Item>>): void {
    ...
    for (const [cell, keys] of fresh) {
      ...
      namesHere.add(id)
      if (this.doc.getArray(CELLS_KEY).toArray().some((c) => c instanceof Y.Map && c.get('id') === id)) {
      ...
      if (!isType(keys.get('source'), Y.Text)) throw new Refusal('текст новой ячейки не Y.Text', path)
      if (!isType(keys.get('outputs'), Y.Array)) throw new Refusal('вывод новой ячейки не Y.Array', path)

      const state = valueOf(keys.get('state'))
      if (state !== 'idle') throw new Refusal('новая ячейка объявляет себя выполнявшейся', path)
      for (const key of ['execCount', 'runBy', 'runById', 'startedAt', 'ranMs']) {
        const value = keys.get(key)
        if (value && valueOf(value) !== null) {
          throw new Refusal(`новая ячейка объявляет «${key}»`, path)
        }
      }
    }
  }
  ...
  judge(): { verdicts: Verdict[]; retyped: string[] } {
...
export function classify(doc: Y.Doc, payload: Uint8Array): Judgement {
  ...
  try {
    const dec = Y.decodeUpdate(payload) as unknown as { structs: Struct[]; ds: DeleteSet }
    const { verdicts, retyped } = new Frame(doc, dec).judge()
    return { ok: true, verdicts, retyped }
  } catch (err) {
