...
restore: ## Развернуть снятую копию: базу и файлы семинаров. Без аргументов — свежую
...
##
## — арендовать, развернуть, развернуть копию из backups/ и открыть адрес наружу.
## Без HOST и GPU всё как раньше: карта из VAST_* в .env, адрес не поднимается.

vast-up: ## Арендовать машину с GPU и развернуть Colloq. HOST=имя GPU="RTX 5070"
	@# HOST и GPU уходят окружением, а не аргументами, — как COLLOQ_HOSTNAME в
	@# `make host`. Пустые они и означают «не просили»: скрипт тогда ведёт себя
	@# ровно как до их появления.
	@HOST="$(HOST)" GPU="$(GPU)" ./scripts/vast.sh up

vast-status: ## Что арендовано, живо ли оно, отвечает ли адрес и сколько натикало
	@./scripts/vast.sh status

vast-sync: ## Снять данные с арендованной машины сюда
	@./scripts/vast.sh sync

vast-down: ## Уничтожить арендованную машину — вместе со всем, что на ней
	@./scripts/vast.sh down
