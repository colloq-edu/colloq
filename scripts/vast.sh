...
if curl -sf -o /dev/null --max-time 3 "http://localhost:${port_here:-3000}/api/health" 2>/dev/null; then
  ...
  # контейнере.
  FORM=service bash scripts/restore.sh "backups/$WANT_DB"
else
...
# него не встанет.
#
# Строго ПОСЛЕ решения про копию, и это не порядок ради порядка: «здесь уже
# работает» выясняется по ответу на localhost, а остановленный app не отвечает.
# Стоя выше, эти три строки превращали живую машину в «пустую» и разворачивали
# на неё вчерашнюю копию поверх сегодняшнего семинара.
#
