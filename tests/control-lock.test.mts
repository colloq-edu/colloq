 ...
 *
 * Ни сети, ни ядра: сокет поддельный, комната настоящая. Обещания, что промах
 * проверки «упал бы попыткой сходить в несуществующий Jupyter», здесь больше
 * нет — оно было неправдой: `JUPYTER_URL` смотрит в мёртвый порт, отказ
 * соединения глотается, и тест остаётся зелёным. Промах ловят утверждения: то,
 * что отказ прозвучал, и то, что документ комнаты после него не изменился.
 */
import './_env.mts'
