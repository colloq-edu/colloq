...
test('отказ на одном пути не оставляет комнату наполовину переехавшей', () => {
  /*
   * Заметки — единственное в каскаде, что ходит в базу, и там у переезда есть
   ...
   * завели файл с прежним именем — и `UPDATE lecture_notes SET file` падает
   * посреди цикла. Раньше исключение обрывало каскад целиком: файлы на диске
   * уже переехали, а тетрадь, доска и вкладки оставались на старых путях, и
   * человек получал английскую строку SQLite вместо объяснения.
   */
  const id = room()
  const { doc } = getSessionDoc(id)
  ...
  /*
   * А вот заметки к этой странице потеряны, и это не наша дыра, а `moveNotesTo`
   * в server/src/db.ts: перенос идёт голым `UPDATE` по таблице с первичным
   * ключом. Чинится там — одной транзакцией «удалить строки цели, перенести
   * строки источника»; строка стоит здесь, чтобы в тот день упасть.
   */
  assert.deepEqual(notesOf(id, 'курс/a.pdf'), { 7: 'речь к прошлому черновику' })
  closeControlRoom(id)
})
