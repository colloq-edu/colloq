...
test('обе поверхности рисуют строку на письмо, а не абзац на попытку', () => {
  for (const [name, sources, mark] of [
    ...
    // В пульте рассылка названа «Вы · письмо группе»: у преподавателя это его
    // собственное письмо, и оно обязано отличаться от личного. Числа людей в
    // подписи больше нет — группировку из пульта убрали 20.09 вместе с кнопкой
    // «Всем N», а придумывать размер группы, которого пульт уже не считает,
    // значило бы печатать выдуманное число.
    ['пульт преподавателя', [WORK, LETTERS], /tr\('room\.pult\.v3\.letterToGroup'\)/],
  ] as const) {
