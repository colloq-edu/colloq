...
<Section title={tr('dependencies.adminTitle')} description={tr('dependencies.adminHint')}>
  <div class="space-y-5 text-[15px] leading-6">
    {#if !data}<p role="status">{tr('dependencies.loading')}</p>{:else}
      {#if data.capabilities?.preparation.available === false}<p class="text-warning" role="status">{data.capabilities.preparation.reason}</p>{/if}
      <label class="flex items-start gap-3 text-ink"><input class="mt-1 size-4 accent-accent" type="checkbox" bind:checked={enabled} disabled={busy || data.capabilities?.preparation.available === false} /><span>{tr('dependencies.enabled')}</span></label>
