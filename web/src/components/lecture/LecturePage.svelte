...
<script lang="ts">
  ...
  $effect(() => {
    ...
    void (async () => {
      ...
      if (dropped) return
      /*
       * СТРАНИЦУ БЕРЁМ С ПОВТОРАМИ.
       *
       * `getPage` — это не «достать из массива», а «дождаться, пока приедут
       * данные этой страницы»: документ качается по кускам, и на девятой
       * странице тридцатимегабайтной колоды кусок вполне может ещё не доехать.
       * Отказ был молчаливым и окончательным: одна неудачная попытка — и
       * эффект больше не звали, потому что звать его нечему, все зависимости на
       * месте. Проекция оставалась на прошлом слайде, пока что-нибудь
       * постороннее её не толкнёт. На стенде это видно буквально: «страницы
       * нет» на 256-й миллисекунде и следующая попытка только на 1066-й — и то
       * лишь потому, что приехала пропорция и пересчиталась раскладка.
       *
       * Пять попыток за секунду с небольшим — это про сеть, а не про
       * терпение: если страницы нет и после них, дело не в задержке, и
       * следующего повода эффект дождётся сам.
       */
      let sheet: Awaited<ReturnType<typeof source.getPage>> | null = null
      for (let tries = 0; tries < 5 && !dropped; tries += 1) {
        sheet = await source.getPage(index).catch(() => null)
        if (sheet || dropped) break
