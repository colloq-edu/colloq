 ...
 * сразу: умолчать про стёртые вопросы, умолчать про то, что они возвращаются,
 * и пообещать герметичность, которой нет. Имя не склоняется ни в одной из
 * строк: «Ивана» и «Ксении» одинаково правильно получаются только у человека.
 */
export function banConsequences(name: string): string[] {
  return [
    `${name} не войдёт в комнату сутки — ни тетради, ни терминала, ни оракула.`,
    'Вопросы этого человека к оракулу пропадут из общей ленты.',
    'Перед удалением комната отметит историю версий: восстановлением версии вопросы вернутся.',
    'Метка держится на браузере — из инкогнито или с другого ноутбука человек войдёт снова.',
  ]
}
 ...
 * «впервые, только что» стоит рядом с любой из них — вместе они и складываются
 * в того, кого преподаватель ищет глазами.
 *
 * Догадка не показывается, пока в комнате никого не удаляли: совпавший адрес
 * сам по себе не значит ничего — за одним вайфаем сидит вся аудитория.
 */
export function personNotes(
  ...
  if (mark.device === false && mark.sameIp === true && options.bansActive) {
    notes.push({
      text: 'возможно, вернулся',
      why:
        'Браузер без метки устройства, а адрес тот же, что у удалённого с занятия. ' +
        'Это догадка: она никого не блокирует и легко ошибается — за одним вайфаем ' +
        'сидит вся аудитория.',
    })
  } else if (mark.device === false) {
    notes.push({
      text: 'браузер без метки',
      why: 'Вкладка не приняла метку устройства — так выглядит инкогнито. Бан на таком браузере не удержится.',
    })
  }

  if (mark.firstSeenAt !== undefined && now - mark.firstSeenAt < FRESH_MS) {
    notes.push({
      text: 'впервые, только что',
      why: 'Первый раз в этой комнате — вошёл несколько минут назад.',
    })
  }
