...
ядра, порты, туннель, остановка — живёт в CLI (_app/cli/colloq.mjs) и должна
быть одна на все способы установки: из репозитория `./colloq`, из колеса
`colloq`. Второй, питонский, слой правил означал бы, что у преподавателя и у
разработчика разные программы, и чинить пришлось бы обе.

Что шим делает по шагам:

  1. ищет node — сначала в PATH, потом там, где его кладут установщики;
     проверяет, что версия не ниже 20 (сервер собран под node20, и на 18 он
     падает на ходу, а не при старте);
  2. если рядом с приложением нет node_modules — ставит их один раз npm-ом и
     ...
     «зависло»;
  3. выставляет COLLOQ_APP_DIR (где лежит приложение) и COLLOQ_SURFACE=teacher
     (какую поверхность команд показывать);
...
#: Ниже 20 не бывает: сервер и CLI собраны под node20 (см. server/package.json).
MIN_NODE = 20
...
def main() -> int:
    ...
    # Поверхность преподавателя: команды разработки и аренды машин из неё
    # убраны. setdefault, а не присваивание: тот, кто позвал шим с явным
    # COLLOQ_SURFACE, знает, что делает, и отнимать у него это незачем.
    environment.setdefault("COLLOQ_SURFACE", "teacher")
    # Каталог, из которого позвали, — тем же именем, что в обёртке ./colloq:
    # относительные пути в аргументах должны считаться от него, а не от
    # приложения.
