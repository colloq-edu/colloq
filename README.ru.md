...
* **консилиумом** — у каждой такой ячейки свой порядок запуска: **«Только я»**
  ...
  Данные каждая попытка получает свои: сервер даёт ей личные копии, а после
  возвращает пространство имён в точности к тому, что было — снимает заведённые
  ею имена и отменяет перепривязки, так что `data = data.dropna()` или
  `df.drop(..., inplace=True)` у одного не доходят до следующего. Всё остальное
  остаётся общим: файлы на диске, состояние модулей (`np.random.seed`,
  `pd.set_option`), объекты типов, которые не копируются, и объекты сверх
  бюджета `COUNCIL_COPY_MB` — о последних попытка читает в собственном выводе.
