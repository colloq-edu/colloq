...
**Docker** (macOS и Linux; в Windows — через WSL 2). На macOS 14+ и Linux
x64/arm64 пакет привозит свой Node.js; на остальных системах нужен
**Node.js 22 или новее**. Сервер, веб-приложение и образ комнаты приезжают
вместе с пакетом, а ваши занятия живут в `~/.colloq`.
...
останавливается сама; исполняются ли опасные команды; кто меняет состав тетради;
кто показывает документ комнате; кто создаёт и редактирует файлы; может ли
студент завести свою тетрадь; правит ли оракул файлы комнаты сам; сколько
...
Соревнование — это задача с закрытой проверкой: вы даёте классу данные и
описание, участник присылает тетрадь `.ipynb` файлом, а сервер исполняет её **с
нуля** в одноразовом контейнере без сети и считает вашей метрикой результат.
Живёт оно рядом с курсами, в разделе **«Соревнования»** панели, и открывается
классу по адресу вида `/k/<имя>`.

Что вы заводите: название и адрес, описание в Markdown, открытые файлы данных,
**скрытые ответы**, сэмпл-тетрадь с базовым решением, код метрики, пределы
исполнения и дедлайн. Пока сэмпл-тетрадь не прошла весь путь до числа,
соревнование не открыть: задача, которая не решается даже у автора, — это сто
человек, безуспешно ищущих ошибку у себя.

Как считается результат. Посылка проходит два шага в двух разных одноразовых
контейнерах: сначала исполняется тетрадь и из неё забирается `submission.csv`,
потом отдельно запускается ваша функция `score(solution, submission)`. Метрика
считается **дважды** — на публичной части строк и на приватной. Публичный
лидерборд виден всегда, итоговый открывается на дедлайне. В зачёт идёт посылка,
которую выбрал участник; не выбрал — лучшая по публичной части.

Что видит участник. Свои посылки и только свои, движение этапов вживую
(«ПРИНЯТА → ОЧЕРЕДЬ → ЗАПУСК ТЕТРАДИ → ПРОВЕРКА CSV → ОЦЕНКА»), свою
трассировку дословно, если тетрадь упала, и своё место. Текст исключения
`ParticipantVisibleError` из вашей метрики он читает целиком — всё остальное,
что она может напечатать, видите только вы.

Личность участника — уровня инстанса, а не комнаты: человек получает **ключ
входа** вида `K7Q-M2X-9FD` и ссылку к нему, и тем же ключом возвращается к своим
посылкам с другого устройства. В базе ключа нет: лежат его отпечаток и
зашифрованная секретом инстанса копия. Преподаватель может выдать новый ключ —
старый перестаёт действовать в ту же секунду вместе со всеми открытыми вкладками.

Очередь — одна на инстанс, живёт в SQLite и переживает перезапуск сервера. По
умолчанию исполняется одна посылка за раз, очередь честная по людям (вторая
посылка того же человека встаёт за первой посылкой остальных), а преподаватель
может её приостановить и убить идущий прогон.

Руководство: [соревнования](https://colloq.ru/docs/competitions.html)
...
Машине нужен запущенный **Docker**. На macOS 14+ и на Linux x64/arm64
(glibc 2.28+) pip ставит колесо, в котором уже есть Node.js и зависимости
сервера, — больше ничего не нужно. На остальных системах (macOS 13, Alpine,
другие архитектуры) pip берёт универсальное колесо, и ему нужен
**Node.js 22 или новее**: `colloq` сам находит его, в том числе под nvm, fnm,
volta и asdf, а если его нет — говорит об этом словами; на первом запуске он
один раз ставит зависимости сервера. Ctrl+C сохраняет тетрадь и заканчивает
занятие.

1. Откройте ссылку, которую печатает `colloq start`. Как у Jupyter, в ней токен
   установки (он же лежит в `~/.colloq/data/setup-token`): в первый раз она
   делает вас владельцем инстанса, а потом просто входит.
...
| `COMPETITION_BACKEND` | Каким путём исполняются посылки соревнований: `docker` (два одноразовых контейнера на посылку — тетрадь и метрика) или `test` — подставной прогонщик, который не исполняет ни строки из присланной тетради. Пусто — `docker`; `test` требует `NODE_ENV=test`, ровно как `KERNEL_BACKEND`. Подставной путь и есть то, на чём весь продукт — очередь, лидерборд, выбор посылки в зачёт — работает на машине без docker. Исполнение соревнований и подготовка пакетов поддерживаются на локальном Docker-пути. В стандартной k3s/broker-поставке исполнителя соревнований нет: действия заранее недоступны, причина видна в интерфейсе и capabilities `/api/health`. Подключать Docker socket к production-приложению для обхода этой проверки не следует. |
| `DATA_HOST_DIR` | Каталог соревнований глазами демона docker — то же, что `WORKSPACE_HOST_DIR` для комнат. На хозяйском сервере оставьте пустым. Под `make up` сервер сам сидит в контейнере, и без этой переменной посылка молча получит пустой каталог вместо данных. |
...
соревнования, окружения, публикации, сеть, резервные копии и обновления — по-русски и
