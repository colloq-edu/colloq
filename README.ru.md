...
### Семинар: пишем вместе, ядро одно

Все правят одни и те же ячейки вживую и видят, кто где печатает. У занятия
**одно ядро Python**: запуски встают в видимую очередь, вывод доходит до всех, а
переменная, которую завёл один студент, есть и у следующего. Кто вошёл позже,
видит тетрадь такой, какая она сейчас. Это формат **«Обычный»**.
...
Попытки считаются в том же единственном ядре комнаты, поэтому консилиум — приём
...
* **консилиумом** — у каждой такой ячейки свой порядок запуска: **«Только я»**
  ...
  два способа кончить занятие всем сразу: `exit()` и `quit()` в попытке
  отказывают (в ядре Jupyter они завершают процесс, и переменные теряют все), а
  адресное пространство попытки ограничено (`COUNCIL_MEMORY_GUARD`), так что
  `np.ones((40000, 40000))` падает `MemoryError` на своей карточке, а не зовёт
  OOM-killer на ядро комнаты. Возвращается и состояние процесса, которое иначе
...
печатает в ячейках; кто запускает код; кто меняет состав тетради; кто показывает
документ комнате; кто создаёт и редактирует файлы; правит ли оракул файлы
комнаты сам; сколько действий у одного запроса; сколько вопросов в час; какой
промежуток между вопросами; кто смотрит ленту версий; кто перезапускает ядро; и
кто очищает общие результаты. Источник этих настроек — [rule-rows.ts](web/src/lib/rule-rows.ts).
...
| `COUNCIL_MEMORY_GUARD` | Ограничивать ли адресное пространство попытки консилиума пределом контейнера за вычетом занятого (`1` по умолчанию, `0` выключает). Жадная попытка тогда падает с `MemoryError` на своей карточке, а не зовёт OOM-killer, который уносит ядро комнаты вместе с переменными всех. Там, где это не работает, не ставится само: не Linux, нет предела cgroup, рядом CUDA. |
