 ...
 */
export type InspectMiss = 'no-kernel' | 'starting' | 'busy' | 'unknown' | 'refused'
 ...
 *
 * Обновляется ТОЛЬКО рукой, кнопкой «Обновить». Модель видела тексты по
 * группам с числами, задание и эталон, имён не видела: `groupLabels`/`drafts`
 * идут по ключу группы, а к людям их привязывает клиент.
 *
 * «Сводка отстала» СЧИТАЕТСЯ КЛИЕНТОМ и в этом кадре не приезжает.
 * `basedOn` — сколько попыток модель читала; сдано на экране столько-то;
 * разница и есть «с тех пор сдали ещё N» (web/src/lib/council-board.ts ·
 * oracleState/staleBy). Иначе сервер обязан был бы слать `council:oracle` на
 * каждое чужое «Сдать» — то есть ровно то, что этот кадр не делает намеренно.
 */
/**
 * Один ход ленты «вопрос → ответ»: что спросили о классе и что ответили.
 *
 * Отдельно от `summary`, а не вместо неё: сводка по решениям — это разбор кода
 * трёх групп, и она живёт до следующего «Обновить»; вопрос о классе — разговор
 * на минуту («кто застрял?»), и он копится лентой. Одно поле на двоих означало
 * бы, что вопрос про тишину стирает три абзаца, которые преподаватель читал
 * перед этим.
 *
 * `people` — вот ради чего всё: модель видит людей метками S1…SN и других имён
 * не знает (server/src/ai/council.ts · statusPrompt), а пульт по этому словарю
 * превращает `S7` в чипе в имя студента (или «Вариант N» при выключенных
 * именах) и в кнопку, открывающую его работу. Словарь считает СЕРВЕР при сборке
 * кадра: угадать по ответу, кто такой S7, клиент не может — нумерация живёт
 * ровно один вопрос.
 *
 * Поэтому же этот кадр едет ТОЛЬКО преподавателям (control.ts · toTeachers):
 * `people` — это соответствие «метка → participantId», то есть разметка чужих
 * работ, которой в классе не место.
 */
/**
