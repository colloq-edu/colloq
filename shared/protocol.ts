...
export type ControlClientMessage =
   ...
   */
  | { t: 'complete'; id: number; code: string; cursor: number }
  /** Справка о том, что стоит под кареткой: сигнатура для подсказки над скобкой. */
  | { t: 'inspect'; id: number; code: string; cursor: number }
  | { t: 'ping' }
 ...
 *
 * Одна копия правила на обоих читателей: лист студента (CellView) и стопка
 * преподавателя (CouncilStack) рисуют письма построчно, и оба обязаны уметь
