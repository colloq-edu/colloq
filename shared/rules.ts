 ...
 *
 * Меняются РОВНО `run`, `edit` и `structure`. Всё остальное — ядро, терминал,
 * доска, файлы, история, оракул, потолки — общее для комнаты, и у него нет
 * измерения «в какой тетради».
 *
...
export function rulesForBook(rules: RoomRules, root: string | null, who: Asker): RoomRules {
  ...
  const mine = rule.access === 'all' || (rule.access === 'owner' && ownsBook(rule, who))
  /*
...
/**
 * Считает ли эта тетрадь СВОИМ ядром.
 *
 * Сегодня — ни одна: запуск у всех тетрадей идёт в ядро комнаты, и эта функция
 * никуда не маршрутизирует. Она отвечает на вопрос «чья это работа», который у
 * личной тетради имеет ответ, отличный от комнатного: `owner` — это тетрадь
 * одного человека, и считаться она должна там же, где живёт, а не в общем
 * пространстве имён занятия.
 *
 * Заведена сейчас и отдельной функцией ровно затем, чтобы шаг, который заведёт
 * личной тетради собственный контейнер, спрашивал ОДНО место, а не рассыпал по
 * коду сравнение `access === 'owner'`: второй копией такого вопроса и
 * отличается разошедшееся правило от правила.
 *
 * `all` и `host` сюда не попадают намеренно: «открыта всем» — это по-прежнему
 * общая тетрадь занятия, а «только преподаватель» — тем более.
 */
