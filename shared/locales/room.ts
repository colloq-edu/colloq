...
export const roomMessages: MessageCatalog = {
  ...
  "room.rules.ownBooks.note": {"ru": "Может ли студент завести в комнате свою тетрадь — пустую или внеся .ipynb из папки. Своя тетрадь всегда личная: правят и запускают автор и преподаватель, остальные смотрят. По умолчанию выключено. Не то же, что строка выше: та про общую папку занятия. Не больше трёх своих тетрадей на человека; свою автор может и убрать.", "en": "Whether a student may start their own notebook in the room — empty, or by bringing in an .ipynb from the folder. An own notebook is always personal: its author and the teacher edit and run it, everyone else watches. Off by default. Not the same as the row above, which is about the class’s shared folder. At most three own notebooks per person; the author may remove their own."},
