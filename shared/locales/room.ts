...
export const roomMessages: MessageCatalog = {
  ...
  "room.book.access.roomHint": {"ru": "Правила занятия — как есть", "en": "The class rules, unchanged"},
  "room.book.access.owner": {"ru": "Личная — {name}", "en": "Personal — {name}"},
  "room.book.access.ownerNoAuthor": {"ru": "Личная", "en": "Personal"},
  "room.book.access.ownerHint": {"ru": "Печатает, меняет состав и запускает автор; остальные смотрят", "en": "The author types, rearranges and runs; everyone else watches"},
  "room.book.access.ownerNone": {"ru": "Автор есть только у своей тетради студента — эту заводили не так", "en": "Only a student’s own notebook has an author — this one was started differently"},
  "room.book.access.all": {"ru": "Открыта всем", "en": "Open to everyone"},
  "room.book.access.allHint": {"ru": "Печатает и запускает любой, что бы ни говорили правила комнаты", "en": "Anyone types and runs, whatever the room’s rules say"},
  "room.book.access.host": {"ru": "Только преподаватель", "en": "Teacher only"},
  "room.book.access.hostHint": {"ru": "Печатает и запускает только преподаватель, что бы ни говорили правила комнаты", "en": "Only the teacher types and runs, whatever the room’s rules say"},
  "room.book.mark.mine": {"ru": "моя", "en": "mine"},
  "room.book.mark.personal": {"ru": "личная · {name}", "en": "personal · {name}"},
  "room.book.mark.personalNoName": {"ru": "личная", "en": "personal"},
  ...
  "room.signature.module": {"ru": "модуль", "en": "module"},
  "room.signature.submodule": {"ru": "модуль пакета {p0} {p1}", "en": "module of {p0} {p1}"},
  "room.signature.docs": {"ru": "Документация:", "en": "Documentation:"},
  "room.signature.thinking": {"ru": "Разбираю библиотеку — это бывает долго только в первый раз", "en": "Reading the library — slow only the first time"},
  ...
  "room.pult.v3.runFailedAt": {"ru": "Ошибка запуска · {time}", "en": "Execution error · {time}"},
  "room.pult.v3.runRunning": {"ru": "Считает {duration}", "en": "Running {duration}"},
  "room.pult.v3.runStopped": {"ru": "Остановлен пределом {limit} · {time}", "en": "Stopped by the {limit} limit · {time}"},
  "room.pult.v3.runQueued": {"ru": "В очереди с {time}", "en": "Queued since {time}"},
  "room.pult.v3.ranAt": {"ru": "запущен в {time}", "en": "started at {time}"},
  "room.pult.v3.queueSince": {"ru": "ждёт с {time}", "en": "waiting since {time}"},
  "room.pult.v3.queueRunning": {"ru": "запущен в {time} · идёт {duration}", "en": "started at {time} · running for {duration}"},
  "room.pult.v3.codeAll": {"ru": {"one": "Показать весь код · {count} строка", "few": "Показать весь код · {count} строки", "many": "Показать весь код · {count} строк", "other": "Показать весь код · {count} строки"}, "en": {"one": "Show all code · {count} line", "other": "Show all code · {count} lines"}},
  "room.pult.v3.more": {"ru": "Ещё действия", "en": "More actions"},
  "room.pult.v3.place": {"ru": "{index} / {total}", "en": "{index} / {total}"},
  "room.pult.v3.backToList": {"ru": "Работы", "en": "Work"},
  "room.pult.v3.prevWork": {"ru": "Предыдущая работа", "en": "Previous work"},
  "room.pult.v3.nextWork": {"ru": "Следующая работа", "en": "Next work"},
  "room.pult.v3.openSearch": {"ru": "Найти по имени", "en": "Find by name"},
  "room.pult.v3.letters": {"ru": "Переписка с автором", "en": "Conversation with the author"},
  "room.pult.v3.exit": {"ru": "Вернуться в тетрадь", "en": "Back to the notebook"},
  "room.pult.v3.shortShow": {"ru": "Классу", "en": "Show"},
  "room.pult.v3.shortClear": {"ru": "Убрать", "en": "Clear"},
  ...
  "room.pult.v3.shareTitle": {"ru": "Пульт консилиума", "en": "Council console"},
  "room.pult.v3.shareDanger": {"ru": "За ссылкой — работы всего класса целиком: имена, черновики, ошибки и отметки. Не пересылайте её никому.", "en": "Behind this link is the whole class’s work: names, drafts, errors and marks. Do not forward it to anyone."},
  "room.pult.v3.queue.scopeBook": {"ru": "Очередь ядра этой тетради · код выполняется по одному", "en": "This notebook’s kernel queue · code executes one at a time"},
  "room.pult.v3.queue.countsBook": {"ru": "{running} выполняется · {queued} в очереди тетради", "en": "{running} running · {queued} in the notebook queue"},
  "room.pult.v3.queue.noQueuedBook": {"ru": "Очередь тетради пуста", "en": "The notebook queue is empty"},
  "room.pult.v3.queue.busyCell": {"ru": "Ядро занято ячейкой {index}", "en": "The kernel is busy with cell {index}"},
  "room.pult.v3.queue.busyCellPlain": {"ru": "Ядро занято ячейкой тетради", "en": "The kernel is busy with a cell of the notebook"},
  "room.pult.v3.queue.busyOther": {"ru": "Ядро занято попыткой из ячейки {index}", "en": "The kernel is busy with an attempt from cell {index}"},
  "room.pult.v3.queue.busyOtherPlain": {"ru": "Ядро занято попыткой из другой ячейки", "en": "The kernel is busy with an attempt from another cell"},
  "room.pult.v3.queue.holds": {"ru": "Пока это считается, очередь попыток стоит", "en": "While this runs, the attempt queue waits"},
  "room.pult.v3.queue.idleBook": {"ru": "Ядро этой тетради свободно", "en": "This notebook’s kernel is free"},
  "room.pult.v3.queue.stuck": {"ru": "Ядро не отвечает на прерывание", "en": "The kernel is not answering the interrupt"},
  "room.pult.v3.queue.stuckWhy": {"ru": "Два сигнала остановки ушли, а работа всё считается: Python ушёл в код, который сигнала не видит. Очередь тетради не двинется сама.", "en": "Two stop signals have gone out and the job is still running: Python is inside code that never sees them. The notebook queue will not move on its own."},
  "room.pult.v3.queue.restart": {"ru": "Перезапустить ядро тетради", "en": "Restart the notebook kernel"},
  "room.pult.v3.queue.restartCost": {"ru": "Переменные сбросятся у всех, кто работает в этой тетради, — и у вас.", "en": "Variables reset for everyone working in this notebook — including you."},
  "room.pult.v3.queue.openRow": {"ru": "Открыть работу · {name}", "en": "Open the work · {name}"},
  "room.pult.v3.queue.rowMenu": {"ru": "Что сделать с этим запуском", "en": "What to do with this run"},
  "room.pult.v3.queue.drop": {"ru": "Снять запуск с очереди", "en": "Take the run out of the queue"},
  "room.pult.v3.queue.dropRunning": {"ru": "Запуск уже считается — его прерывают кнопкой «Прервать»", "en": "The run is already going — the Interrupt button stops it"},
  "room.pult.v3.queue.waitingFor": {"ru": "ждёт {duration}", "en": "waiting {duration}"},
  "room.rules.cellLimit.title": {"ru": "Ячейка останавливается сама", "en": "A cell stops itself"},
  "room.rules.cellLimit.note": {"ru": "Через сколько секунд сервер прерывает выполнение ячейки. Пусто — никогда, и так по умолчанию. Ядро у тетради одно, а очередь к нему общая: бесконечный цикл в одной ячейке держит и остальные ячейки, и все попытки консилиума. Предел касается и ваших ячеек.", "en": "After how many seconds the server stops a running cell. Empty means never, and that is the default. A notebook has one kernel and one shared queue: an endless loop in a single cell holds up every other cell and every council attempt. The limit applies to your own cells too."},
  "room.rules.cellLimit.unit": {"ru": "с", "en": "s"},
  "room.rules.danger.title": {"ru": "Опасные команды", "en": "Dangerous commands"},
  "room.rules.danger.note": {"ru": "exit(), os._exit(), os.kill по самому ядру, !kill -9 -1, shutdown, rm -rf корня занятия, %reset и %xdel. В общей тетради одна такая строка роняет ядро или стирает переменные сразу у всех — по умолчанию они не исполняются, и правило касается ваших ячеек тоже. Выключайте, если ведёте курс по самому Python и показываете эти команды в работе. Это лежачий полицейский, а не песочница: знающий его обойдёт, и терминал комнаты он не закрывает. В попытке консилиума такие команды не исполняются в любом случае.", "en": "exit(), os._exit(), os.kill aimed at the kernel itself, !kill -9 -1, shutdown, rm -rf of the class folder, %reset and %xdel. In a shared notebook one such line drops the kernel or wipes everyone’s variables at once — by default they do not run, and the rule applies to your own cells too. Turn it off if you teach Python itself and demonstrate these commands. It is a speed bump, not a sandbox: anyone who knows how will get around it, and it does not close the room’s terminal. Inside a council attempt such commands never run either way."},
  "room.rules.danger.block": {"ru": "Не исполняются", "en": "Blocked"},
  "room.rules.danger.allow": {"ru": "Исполняются", "en": "Allowed"},
  "room.pult.v3.rules.cellLimitOff": {"ru": "Обычные ячейки этой тетради сейчас без предела: один бесконечный запуск задержит всю очередь попыток. Предел ячейки включается в правилах комнаты.", "en": "Ordinary cells in this notebook have no limit right now: one endless run will hold the whole attempt queue. The cell limit lives in the room rules."},
  "room.pult.v3.rules.cellLimitOn": {"ru": "Обычная ячейка этой тетради останавливается сама через {duration} — по правилам комнаты.", "en": "An ordinary cell in this notebook stops itself after {duration} — by the room rules."},
  "room.pult.v2.rules.pauseWhyEveryone": {"ru": "Сколько студент ждёт после своего запуска, прежде чем встать в очередь снова. При «все по очереди» пауза — единственное, что мешает занять очередь повторными нажатиями после каждой правки. Вас она не касается.", "en": "How long a student waits after their own run before queueing again. With “everyone in turn” the pause is the only thing stopping the queue from filling with repeat presses after every edit. It does not apply to you."},
  "room.pult.v3.cells.open": {"ru": "Выбрать ячейку консилиума", "en": "Choose a council cell"},
  "room.pult.v3.cells.title": {"ru": "Ячейки консилиума", "en": "Council cells"},
  "room.pult.v3.cells.submitted": {"ru": "сдали {submitted} из {total}", "en": "{submitted} of {total} submitted"},
  "room.pult.v3.cells.running": {"ru": "идёт запуск", "en": "running"},
  ...
  "room.kernel.state.off": {
    "ru": "НЕ ЗАПУЩЕНО",
    "en": "NOT RUNNING"
  },
  "room.kernel.state.offWhy": {
    "ru": "Ядро этой тетради поднимется при первом запуске ячейки — или когда вы наведёте на имя за справкой",
    "en": "This notebook’s kernel starts with the first cell you run — or when you hover a name for help"
  },
